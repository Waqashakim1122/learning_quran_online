...
<head>
    ...
    <title><?php echo htmlspecialchars($title ?? 'All Enrollments'); ?> - Admin</title>
    ...
    <style>
        /* Custom styles for the dashboard and enrollments page */
        body {
                ...
                <div class="row">
                    <main class="col-12 px-md-4">
                        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb bg-transparent p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page"><?php echo htmlspecialchars($title ?? 'All Enrollments'); ?></li>
                                </ol>
                            </nav>
                        </div>

                        <h1 class="display-5 fw-bold text-dark mb-4"><?php echo htmlspecialchars($title ?? 'All Enrollments'); ?></h1>
                        ...
                        <div class="card shadow-sm border-0 mb-4">
                            ...
                            <div class="card-body">
                                <form method="get" action="<?= base_url('admin/enrollments/all'); ?>">
                                    <div class="row g-3">
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <a href="<?= base_url('admin/enrollments/all'); ?>" class="btn btn-secondary"><i class="fas fa-undo me-1"></i> Reset</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="card shadow-sm border-0 mb-4">
                            <div class="card-header bg-white py-3 border-bottom">
                                <h5 class="mb-0 fw-bold text-dark">All Enrollments</h5>
                            </div>
                                            ...
                                            <tbody>
                                                <?php foreach ($all_enrollments as $enrollment): ?>
                                                    <tr>
                                                        ...
                                                        <td><?= htmlspecialchars($enrollment->student_email ?? 'N/A'); ?></td>
                                                        <td><?= htmlspecialchars($enrollment->course_name); ?></td>
                                                        <td>
                                                            <span class="badge bg-<?= $enrollment->status == 'active' ? 'success' : ($enrollment->status == 'rejected' ? 'danger' : 'warning'); ?>">
                                                                <?= htmlspecialchars(ucfirst($enrollment->status)); ?>
                                                            </span>
                                                        </td>
                                                        <td><?= date('M d, Y H:i:s', strtotime($enrollment->enrollment_date)); ?></td>
                                                        <td><?= htmlspecialchars($enrollment->assigned_instructor_name ?? 'Not Assigned'); ?></td>
                                                        <td class="text-center pe-3">
                                                            <div class="btn-group">
                                                                ...
                                                                <?php if ($enrollment->status == 'pending_approval'): ?>
                                                                    <a href="<?= base_url('admin/enrollments/approve_enrollment/' . $enrollment->enrollment_id); ?>" class="btn btn-sm btn-success px-3" onclick="return confirm('Approve this enrollment?')">
                                                                        <i class="fas fa-check me-1"></i> Approve
                                                                    </a>
                                                                    <a href="<?= base_url('admin/enrollments/reject_enrollment/' . $enrollment->enrollment_id); ?>" class="btn btn-sm btn-danger px-3" onclick="return confirm('Reject this enrollment?')">
                                                                        <i class="fas fa-times me-1"></i> Reject
                                                                    </a>
                                                                <?php endif; ?>
                                                                <a href="<?= base_url('admin/enrollments/assign/' . $enrollment->enrollment_id); ?>" class="btn btn-sm btn-accent px-3">
                                                                    <i class="fas fa-user-plus me-1"></i> <?= empty($enrollment->assigned_instructor_name) ? 'Assign' : 'Reassign'; ?>
                                                                </a>
                                                            </div>
                                                        </td>
                                                    </tr>
                                ...
                                <?php else: ?>
                                    <div class="card-body">
                                        <div class="alert alert-info text-center">
                                            <i class="fas fa-info-circle me-2"></i> No enrollments found.
                                        </div>
                                    </div>
                                <?php endif; ?>
                            </div>
